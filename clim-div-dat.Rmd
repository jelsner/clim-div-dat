...
cities <- us_cities(map_date = yr, states = "Florida") %>%
  ...
  mutate(Years = yr, Name = Counties$name, FIPS = Counties$fips, Area = as.numeric(st_area(geometry)), popD = as.factor(round(log(population/Area)))) %>%
  uncount(10) %>%
  mutate(Year = rep(seq(yr, yr + 9), times = n_counties)) %>%
  dplyr::select(Name, FIPS, Year, Years, population, Area, popD, geometry)

CITY_Pop <- rbind(CITY_Pop, cities)
}

CITY_Pop$popD[is.na(CITY_Pop$popD)] <- -7
```
...
TestL$YearC <- TestL$Year - 1960

family <- gaussian()
formula <- Temperature ~ YearC + (1 | Month) + (1 | popD)
#formula <- Temperature ~ Year
...
fit1 <- brm(formula = formula,
            ...
            prior = c(set_prior("normal(0, 5)", class = "b"),
                     set_prior("student_t(3, 55, 17)", class = "Intercept"),
