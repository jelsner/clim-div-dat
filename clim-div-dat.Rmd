...
The `gather()` function takes all the values as measured except those named in `key =` argument. All variables are measured (e.g., precipitation in units of hundredths of inches) except `Year`. `Year` is a vector identifying the month.
...
Use the `map()` function in the **purrr** package (part of the **tidyverse**).
```{r}
library(reshape2)
